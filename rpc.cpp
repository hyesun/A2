...
int rpcRegister(char* name, int* argTypes, skeleton f) //server calls
{
    ...
    int msglen = sizeof(server_address) + sizeof(server_port) + MAXFNNAME
            + sizeof(char) + argTypesLen * sizeof(int);
...
void* getClientRequest(void* arg)
{
    while (1)
    {
        ...
        pthread_t thread;
        threadInc();
        if (pthread_create(&thread, NULL, serveClientRequest,
                (void*) &newsockfd))
        {
